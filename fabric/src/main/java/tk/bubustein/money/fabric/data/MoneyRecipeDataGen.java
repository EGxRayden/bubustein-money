...
public class MoneyRecipeDataGen extends FabricRecipeProvider {
    ...
    public static void conversionRecipe(Consumer<FinishedRecipe> consumer, ItemLike output, ItemLike input, @Nullable String string) {
        conversionRecipe(consumer, output, input, string, 1);
    }
    public static void conversionRecipe(Consumer<FinishedRecipe> consumer, ItemLike output, ItemLike input, @Nullable String string, int i) {
        BankMachineRecipeShapelessBuilder.shapeless(output, i).requires(input).group(string).unlockedBy(getHasName(input), has(input)).save(consumer, getConversionRecipeName(output, input));
    }
    public static void fiveItems(Consumer<FinishedRecipe> consumer, ItemLike output, ItemLike input) {
        BankMachineRecipeShapedBuilder.shaped(output).define('#', input).pattern("# #").pattern("###").unlockedBy(getHasName(input), has(input)).save(consumer);
    }
    public static void twoItems(Consumer<FinishedRecipe> consumer, ItemLike output, ItemLike input) {
        twoItemsBuilder(output, Ingredient.of(input)).unlockedBy(getHasName(input), has(input)).save(consumer);
    }
